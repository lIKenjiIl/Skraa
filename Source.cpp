...
//light positions
GLfloat xlight1 = -5.0f, ylight1 = 20.0f, zlight1 = -40.0f;

GLfloat xlight2 = -5.0f, ylight2 = -20.0f, zlight2 = -30.0f;

GLfloat xlight3 = 0.0f, ylight3 = 3.0f, zlight3 = -10.0f;
...
void initRendering() {
	...
	Image* image = loadBMP("texture/pyramid.bmp");
	Pyramid = loadTexture(image);
	delete image;

	Image* image1 = loadBMP("texture/sand.bmp");
	Sand = loadTexture(image1);
	delete image1;

	Image* image2 = loadBMP("texture/sun.bmp");
	_Sun = loadTexture(image2);
	delete image2;

	Image* image3 = loadBMP("texture/moon.bmp");
	_Moon = loadTexture(image3);
	delete image3;

	Image* image4 = loadBMP("texture/space.bmp");
...
int main(int argc, char** argv) {
	...
	//Create the window
	glutCreateWindow("Laurico-EMC122-Fireflies");
	initRendering();
...
void display() {
	...
	glPushMatrix();
	glColor3f(0.5f, 0.5f, 0.5f);//add color material to object //rotation animation
	...
	glColor3f(0.0f, 3.0f, -1.0f);//add color material to object //rotation animation
	glRotatef(_angle, 0.0f, 1.0f, 1.0f);
	glLightfv(GL_LIGHT3, GL_POSITION, light_position3);//set light position
	glLightfv(GL_LIGHT3, GL_DIFFUSE, light3); //add diffuse specular lighting
	glLightfv(GL_LIGHT3, GL_SPECULAR, light3); //add specular lighting
	...
	glColor3f(0.0f, 1.0f, 0.0f);//add color material to object //rotation animation
	glRotatef(_angle, 0.0f, -3.0f, 0.0f);
	glLightfv(GL_LIGHT3, GL_POSITION, light_position3);//set light position
	glLightfv(GL_LIGHT3, GL_DIFFUSE, light3); //add diffuse specular lighting
	glLightfv(GL_LIGHT3, GL_SPECULAR, light3); //add specular lighting
	glMaterialfv(GL_FRONT, GL_EMISSION, mat_emissionYell);
	...
	renderBitmapString(-8.0f, 8.0f, 5.0f, GLUT_BITMAP_HELVETICA_18, "fireflies revolving on the small pine tree");
	...
	glDisable(GL_LIGHTING);
	glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
	renderBitmapString(-8.0f, 6.0f, 5.0f, GLUT_BITMAP_HELVETICA_12, "Press E to play BGM happy");
	glEnable(GL_LIGHTING);

	glDisable(GL_LIGHTING);
	glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
	renderBitmapString(-8.0f, 5.5f, 5.0f, GLUT_BITMAP_HELVETICA_12, "Press R to play BGM cute");
	glEnable(GL_LIGHTING);	
...
void processNormalKeys(unsigned char key, int x, int y) {
	...
	case 'e':
		engine->play2D("sound/happy.wav", false);
		break;
	case 'r':
		engine->play2D("sound/cute.wav", false);
		break;
	}
}
void renderBitmapString(
